...
class vision_dictionary:
    ...
    def feature_extraction(im,gray,SIFT_type,nf):
    ...
    def all_in_one_train(kmeans_k,nf):
        from_scratch=True
        ...
        if from_scratch:
            for j in range(len(names)):
                ...
                for i,image in enumerate(train_data[j]):
                    new_descriptor = vision_dictionary.feature_extraction(image,train_gray_data[j][i],SIFT_type[0],nf)
    ...
    def all_in_one_test(kmeans_k,nf):
        from_scratch=True
        val_data,val_gray_data = Dataloader("validation")
        SIFT_type = ["SIFT","D_SFIT"]
        k=[32,64,128,256]
        ...
        if from_scratch:
            for j in range(len(names)):
                ...
                for i,image in enumerate(val_data[j]):
                    new_descriptor = vision_dictionary.feature_extraction(image,val_gray_data[j][i],SIFT_type[0],nf)
                    new_word = vision_dictionary(names[j],new_descriptor,[])
                    catalog.append(new_word)
                    descriptor = np.concatenate((descriptor,new_descriptor))
...
if __name__=='__main__':
    ...
    nfeatures = [5,15,25,100,1000]
    nOctaveLayers =[100,200,1000]
    contrastThreshold = [0.01,0.02,0.04,0.07,0.09,0.15]
    edgeThreshold = [1,3,5,10,12,15,35,100,200,300,400]
    sigma = [0.001]
    
    for nf in sigma:  #!!!!!! CHANGE
        vision_dictionary.all_in_one_train(kmeans_k,nf)
        catalog_test=vision_dictionary.all_in_one_test(kmeans_k,nf)
        accuracy = vision_dictionary.GO_KNN(catalog_test,knn_k,kmeans_k)
                
        with open('results.txt', 'a') as f:
            #f.write("nfeatures: {}\n".format(nf))
            #f.write("nOctaveLayers: {}\n".format(nf))
            #f.write("edgeThreshold: {}\n".format(nf))
            f.write("sigma: {}\n".format(nf))
            f.write("k for KNN: {}\n".format(knn_k))
            f.write("k for kmeans: {}\n".format(kmeans_k))
            f.write("accuracy: {}\n\n".format(accuracy))
        
        print("sigma: {}".format(nf))        
